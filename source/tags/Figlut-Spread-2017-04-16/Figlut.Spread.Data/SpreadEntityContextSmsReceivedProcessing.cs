namespace Figlut.Spread.Data
{
    ...
    public partial class SpreadEntityContext : EntityContext
    {
        ...
        /// </summary>
        public void DeleteSmsReceivedQueueItem(Guid smsReceivedQueueItemId, bool submitChanges)
        {
        ...
        /// <summary>
        /// * Checks if there's a subscriber for the given Cell Phone Number, if there isn't it will create one. If there is it will update the suscriber's Name of a name is supplied.
            ...
            Nullable<Guid> smsReceivedQueueItemIdToDelete)
        {
            ...
            using (TransactionScope t = new TransactionScope())
            {
                ...
                Subscriber subscriber = SaveSubscriber(smsReceived, true);
                ...
                if (!string.IsNullOrEmpty(smsReceived.OrganizationIdentifier))
                {
                    ...
                    {
                        GOC.Instance.Logger.LogMessage(new LogMessage(string.Format("Could not find {0} with {1} of '{2}'.",
                ...
                {
                    DeleteSmsReceivedQueueItem(smsReceivedQueueItemIdToDelete.Value, true);
                }
                DB.SubmitChanges();
                t.Complete();
            }
        ...
        private Subscriber SaveSubscriber(SmsReceivedParsed smsReceived, bool submitChanges)
        {
            string cellPhoneNumber = smsReceived.From.Trim();
            ...
            if (!string.IsNullOrEmpty(smsReceived.SubscriberName))
            {
                subscriber.Name = smsReceived.SubscriberName;
            }
            if (submitChanges)
            {
                DB.SubmitChanges();
