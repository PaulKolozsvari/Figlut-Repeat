namespace Figlut.Spread.Data
{
    ...
    public partial class SpreadEntityContext : EntityContext
    {
        ...
        public void GetGlobalSettingSmsDaysDateRange(out DateTime startDate, out DateTime endDate)
        {
            ...
            startDate = endDate.Subtract(new TimeSpan(smsDaysToDisplay - 1, 0, 0, 0)); //e.g. if the smsDaysToDisplay is 1, then subtract 0, which will will include only today's SMS'.
        }
        ...
        public void GetGlobalSettingSmsProcessorLogDaysDateRange(out DateTime startDate, out DateTime endDate)
        {
            ...
            int logDaysToDisplay = Convert.ToInt32(setting.SettingValue);
            endDate = DateTime.Now;
            startDate = endDate.Subtract(new TimeSpan(logDaysToDisplay - 1, 0, 0, 0)); //e.g. if the logDaysToDisplay is 1, then subtract 0, which will will include only today's logs'.
        }
