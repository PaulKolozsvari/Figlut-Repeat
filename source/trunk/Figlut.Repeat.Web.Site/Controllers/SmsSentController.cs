namespace Figlut.Repeat.Web.Site.Controllers
{
    ...
    public class SmsSentController : RepeatController
    {
        #region Constants
            ...
            Nullable<Guid> smsCampaignId)
        {
            ...
            int smsErrorTrimLengthOnGrid = Convert.ToInt32(RepeatWebApp.Instance.GlobalSettings[GlobalSettingName.SmsErrorTrimLengthOnGrid].SettingValue);
        ...
        public ActionResult ComposeStandalone(ComposeSmsStandaloneModel model)
        {
            try
            {
                ...
                try
                {
                    response = RepeatWebApp.Instance.SmsSender.SendSms(new SmsRequest(model.CellPhoneNumber, model.MessageContents, maxSmsSendMessageLength, smsSendMessageSuffix, currentOrganization.Identifier, organizationIdentifierIndicator));
                }
                catch (Exception exFailed) //Failed to send the SMS Web Request to the provider.
                {
                    int smsProviderCode = (int)RepeatWebApp.Instance.Settings.SmsProvider;
