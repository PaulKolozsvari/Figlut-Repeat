namespace Figlut.Repeat.Web.Site.Models
{
    ...
    public class WebRequestActivityModel
    {
        ...
        {
            errorMessage = null;
            if (this.WebRequestActivityId == Guid.Empty)
            {
                errorMessage = string.Format("{0} not entered", EntityReader<WebRequestActivityModel>.GetPropertyName(p => p.WebRequestActivityId, true));
            }
