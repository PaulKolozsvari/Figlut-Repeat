    ...
    public class SpreadRestService : RestService, ISpreadRestService
    {
        ...
        public Stream Login()
        {
            ...
            catch (Exception ex)
            {
                ...
                SpreadApp.Instance.EmailSender.SendExceptionEmailNotification(ex);
        ...
        private Organization ValidateOrganizationCredits(SpreadEntityContext context, out User user)
        {
            if (context == null)
            {
                context = SpreadEntityContext.Create();
        ...
        public Stream SendSmsSynchronously(Stream inputStream)
        {
            try
            {
                ...
                int maxSmsSendMessageLength = Convert.ToInt32(SpreadApp.Instance.GlobalSettings[GlobalSettingName.MaxSmsSendMessageLength].SettingValue);
                string smsSendMessageSuffix = SpreadApp.Instance.GlobalSettings[GlobalSettingName.SmsSendMessageSuffix].SettingValue;
                string organizationIdentifierIndicator = SpreadApp.Instance.GlobalSettings[GlobalSettingName.OrganizationIdentifierIndicator].SettingValue;
                ...
                {
                    result.Tag = result.Tag.Substring(0, SpreadApp.Instance.Settings.TrimSmsSentLogResponseTagLength);
                }
        ...
        public Stream SendSmsAsynchronously(Stream inputStream)
        {
            try
            {
                ...
                SmsSentQueueItem result = context.EnqueueSmsSent(
                    ...
                    null,
                    "WEB-SERVICE-REQUEST",
                    (int)SpreadApp.Instance.Settings.SmsProvider,
                    user.UserId,
